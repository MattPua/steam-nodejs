...
describe('UserService', () => {
	...
	test('should get friends list with profiles correctly', async () => {
		...
		expect(friendsList).toBeDefined()
		expect(Array.isArray(friendsList)).toBe(true)
		expect(friendsList.length).toBeGreaterThan(0)
		expect(friendsList[0]).toHaveProperty('steamid')
		expect(friendsList[0]).toHaveProperty('personaname')
		expect(friendsList[0]).toHaveProperty('profileurl')
		expect(friendsList[0]).toHaveProperty('avatar')
		expect(friendsList[0]).toHaveProperty('avatarmedium')
		expect(friendsList[0]).toHaveProperty('avatarfull')
		expect(friendsList[0]).toHaveProperty('friend_since')
	})
})
