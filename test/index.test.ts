...
test('should get number of current players correctly', async () => {
	const steamClient = new SteamClient(apiKey)
	const playerCount =
		await steamClient.stats.getNumberOfCurrentPlayers(testAppId)
...
test('should get recently played games correctly', async () => {
	const steamClient = new SteamClient(apiKey)
	const recentGames =
		await steamClient.player.getRecentlyPlayedGames(steamUserId)
...
test('should get wishlist item count correctly', async () => {
	const steamClient = new SteamClient(apiKey)
	const wishlistCount =
		await steamClient.wishlist.getWishlistItemCount(steamUserId)
